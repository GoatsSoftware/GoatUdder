...
class GoatRepository {
  ...
  async findAll() {
    const result = await query(`
      SELECT g.*, p.name as pad_name, p.location as pad_location
      FROM goats g
      JOIN pads p ON g.pad_id = p.id
  ...
  // Get healthy goats
  async findHealthy() {
    const result = await query("SELECT * FROM goats WHERE health_status = $1 ORDER BY created_at DESC", ['healthy']);
    return result.rows;
  }

  // Create a new goat
  async create(goatData) {
    const { pad_id, name, breed, age, milk_production, health_status } = goatData;
    const result = await query(
      'INSERT INTO goats (pad_id, name, breed, age, milk_production, health_status) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [pad_id, name, breed, age, milk_production, health_status || 'healthy']
    );
  ...
  // Get average milk production by pad
  async getAverageMilkProduction(padId) {
    const result = await query(
      'SELECT AVG(milk_production) as avg_milk FROM goats WHERE pad_id = $1',
      [padId]
    );
