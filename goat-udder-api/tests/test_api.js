...
describe("GoatUdder API - Integration Tests", () => {
  beforeEach(() => {
        ...
        data: [
          {
            id: 1,
            pad_id: 1,
            pad_name: "Pasteur Alpine",
            pad_location: "Pyrénées",
            user_id: 1,
            user_name: "goatlover",
    ...
    mockGoatController.getAverageMilkProduction.mockImplementation(
      (req, res) => {
        res.json({
          success: true,
          data: {
            pad_id: parseInt(req.params.padId),
  ...
  describe("Health Check", () => {
    it("GET /health should return healthy status", async () => {
      ...
      expect(response.body).toEqual({
        status: "healthy",
        service: "goat-udder-api",
      });
    });
  });

  describe("Pis API", () => {
    it("GET /api/udder should return all udder", async () => {
      const response = await request(app).get("/api/udder");
    ...
    it("GET /api/udder/available should return available udder", async () => {
      const response = await request(app).get("/api/udder/available");
    ...
    it("GET /api/udder/:id should return pad by ID", async () => {
      const response = await request(app).get("/api/udder/1");
    ...
    it("GET /api/udder/:id should return 404 for non-existent pad", async () => {
      mockPadController.getPadById.mockImplementation((req, res) => {
        if (req.params.id === "99") {
          return res
            .status(404)
            .json({ success: false, error: "Pad with ID 99 not found" });
        }
        return res.json({
          success: true,
          data: { id: 1, name: "Pasteur Alpine" },
        });
      });

      const response = await request(app).get("/api/udder/99");

      expect(response.status).toBe(404);
      expect(response.body.success).toBe(false);
    });

    it("POST /api/udder should create a new pad", async () => {
      const response = await request(app).post("/api/udder").send({
        name: "New Pad",
        location: "Location",
        capacity: 10,
        price_per_day: 15.0,
    ...
    it("POST /api/udder should return 400 for invalid data", async () => {
      mockPadController.createPad.mockImplementation((req, res) => {
        if (!req.body.name) {
          return res
            .status(400)
            .json({ success: false, error: "Pad name is required" });
        }
        return res
          .status(201)
          .json({ success: true, data: { name: req.body.name } });
      });
    ...
    it("PUT /api/udder/:id should update a pad", async () => {
      const response = await request(app).put("/api/udder/1").send({
        name: "Updated Pad",
        price_per_day: 30.0,
      });
    ...
    it("DELETE /api/udder/:id should delete a pad", async () => {
      const response = await request(app).delete("/api/udder/1");
  ...
  describe("Bookings API", () => {
    ...
    it("POST /api/bookings should create a new booking", async () => {
      ...
      expect(response.status).toBe(201);
      expect(response.body.success).toBe(true);
      expect(response.body.data.total_price).toBe(100.0);
  ...
  describe("Goats API", () => {
    ...
    it("GET /api/goats/pad/:padId/milk-production should return milk production", async () => {
      const response = await request(app).get(
        "/api/goats/pad/1/milk-production",
      );
